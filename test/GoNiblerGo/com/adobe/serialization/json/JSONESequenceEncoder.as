...
//	import mx.utils.ObjectUtil;
	...
	public class JSONESequenceEncoder
	{
		...
		private var cleanObject:Object;
		private var _complexType:Array;
		/**
		...
		public function getCleanObject():Object
		{
			return cleanObject;
		}
...
/*
			...
			{
				var temp:*=a[ i ];
				if(!ObjectUtil.isDynamicObject(temp)&&!ObjectUtil.isSimple(temp))
				{
					var hasComplex:Boolean=false;
...
/*
			...
			if ( classInfo.@name.toString() == "Object" )
			{
				...
				{
					element=item.value;
					 if(!ObjectUtil.isDynamicObject(element)&&!ObjectUtil.isSimple(element))
					 {
						  hasComplex=false;
						 for each( type in _complexType)
						 {
							 if(element is type)
							 {
								 hasComplex=true;
			...
			else // o is a class instance
			{
				for each ( var v:XML in classInfo..*.(name() == "variable"||(name() == "accessor"&& attribute( "access" ).charAt( 0 ) == "r" )) )
				{
					...
					// no comma is necessary
					temp.push({key:v.@name.toString(),value: o[ v.@name ]});
				}
				temp.sortOn("key");
				for  each(  item in temp )
				{
